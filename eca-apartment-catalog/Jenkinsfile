pipeline {
    environment {
        ...
        APP_NAME = "eca-apartment-catalog"
     }
    ...
    stages {
        ...
        stage('Sonar Analysis') {
                    steps {
                       sh "mvn -f $APP_NAME/pom.xml clean package"
                        dir("eca-apartment-catalog"){
                        sh "mvn sonar:sonar \
                        -Dsonar.projectKey=eca-apartment-catalog: \
                        -Dsonar.projectName=eca-apartment-catalog \
        ...
        stage('Building image') {
          steps{
            sh 'docker build -t $REGISTRY_NAME:$BUILD_NUMBER ./eca-apartment-catalog'
          }
        }
